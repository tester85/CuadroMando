...
//declaración del store del grid  
   ...
   var store =new Ext.data.JsonStore({
    url: '?r=clasificacion/Clasificacion', 
    root: 'datos',
...
//declaración del columnModel del grid
 var cm = new Ext.grid.ColumnModel([  
	{id:'id_clasificacion',header:'Identificador', width:7,dataIndex: 'id_clasificacion'},
	{id:'nombre_clasificacion',header: 'Nombre',  width:50,  dataIndex: 'nombre_clasificacion'}
				]); 
...
function winFpGestionar(opcion){
      ...
      case 'Add': {
          ...
          if(!winAddAsociar){
            winAddAsociar = new Ext.Window({modal: true,closeAction:'hide',layout:'fit',
              title:'Adicionar clasificaci&oacute;n',width:250,autoHeight:true,
              buttons:
              [ {                  
                  text:'Cancelar',
                  handler:function(){
       ...
       case 'Mod': {
        if(!winMod){
          winMod = new Ext.Window({modal: true,closeAction:'hide',layout:'fit',
            title:'Modificar clasificaci&oacute;n',width:250,autoHeight:true,
            buttons:
            [{                
                text:'Cancelar',
                handler:function(){
        ...
        Ext.getCmp('nombre_mod').setValue(recordSeleccionado.get('nombre_clasificacion'));  
  ...
  function adicionar(){
    ...
    Ext.Ajax.request({
     url : '?r=Clasificacion/Create',	   
		 params:{ 
			nombre_clasificacion:Ext.getCmp('nombre_add').getValue(),  
				},	
	callback: function(a,b,c)
		{
			if(b)
			{
				...
				if(dd == "") 
				{
					Ext.Msg.show({
				   title:'Informaci&oacute;n de error',
				   msg: 'La clasificaci&oacute;n ya existe',
				   buttons: Ext.Msg.OK,
				   //fn: processResult,
				   animEl: 'elId',
				   icon: Ext.MessageBox.ERROR 
				...
				if(jsonData==true)
				{
					...
					Ext.Msg.show({
							   ...
							   msg: 'Se agreg&oacute; correctamente la clasificaci&oacute;n.',
							   buttons: Ext.Msg.OK,
							   //fn: processResult,
							   animEl: 'elId',
...
function modificarA(){ 
		...
		Ext.Ajax.request({
		url:'?r=Clasificacion/Update',
		method:'POST', 		
		params:
		{   
			id_clasificacion:recordSeleccionado.get('id_clasificacion'),
			nombre_clasificacion:Ext.getCmp('nombre_mod').getValue(), 
		},
		callback: function(a,b,c)
		{
			if(b)
			{
			   ...
			   Ext.Msg.show({
			   title:'Informaci&oacute;n',
			   msg: 'Se modific&oacute; correctamente la clasificaci&oacute;n.',
			   buttons: Ext.Msg.OK,
			   //fn: processResult,
			   animEl: 'elId',
			   icon: Ext.MessageBox.INFO  
  ...
  function eliminarAjax()
	{
   		 Ext.Ajax.request(
			{
      			url : '?r=Clasificacion/Delete',
		 params:{ 
        		id_clasificacion:Ext.getCmp('grid').getSelectionModel().getSelected().get('id_clasificacion')          
				},
			callback:function(a,b,c)
				{
				   if(b)
					{
					   ...
					   if(jsonData==true)
						{ 					
							 Ext.Msg.show({
							   title:'Informaci&oacute;n',
							   msg: 'Clasificaci&oacute;n eliminada correctamente',
							   buttons: Ext.Msg.OK, 
							   animEl: 'elId',
							   icon: Ext.MessageBox.INFO  
						  });
...
//******************************
		var panel = new Ext.FormPanel({
		  width: 904,
			frame:true,	
			applyTo: 'clasificacion',			
			title: 'Gestionar clasificaci&oacute;n',
			bodyStyle:'padding:5px 5px 0',			
			items: [{   
