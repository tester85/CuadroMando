...
//declaración del columnModel del grid
 var cm = new Ext.grid.ColumnModel({
	...
	columns:[  
	{id:'nombre_cargo',header: 'Cargo',  width:50,  dataIndex: 'nombre_cargo'}
				]
	}); 
...
function winFpGestionar(opcion){
      ...
      case 'Add': {
          ...
          if(!winAddAsociar){
            winAddAsociar = new Ext.Window({modal: true,closeAction:'hide',layout:'fit',
              title:'Adicionar cargo',width:440,autoHeight:true,
              buttons:
              [ {                  
                  text:'Cancelar',
                  handler:function(){
       ...
       case 'Mod': {
        if(!winMod){
          winMod = new Ext.Window({modal: true,closeAction:'hide',layout:'fit',
            title:'Modificar cargo',width:440,autoHeight:true,
            buttons:
            [{                
                text:'Cancelar',
                handler:function(){
        ...
        Ext.getCmp('nombre_mod').setValue(recordSeleccionado.get('nombre_cargo'));  
  ...
  function adicionar(){
    ...
    Ext.Ajax.request({
     url : '?r=cargo/Create',	   
		 params:{ 
			nombre_cargo:Ext.getCmp('nombre_add').getValue(),  
				},	
...
function modificarA(){ 
		...
		params:
		{   
			id_cargo:recordSeleccionado.get('id_cargo'),
			nombre_cargo:Ext.getCmp('nombre_mod').getValue(), 
		},
		callback: function(a,b,c)
		{
			if(b)
			{
			   ...
			   Ext.Msg.show({
			   title:'Informaci&oacute;n',
			   msg: 'Cargo modificado correctamente.',
			   buttons: Ext.Msg.OK,
			   //fn: processResult,
			   animEl: 'elId',
			   icon: Ext.MessageBox.INFO  
  ...
  function eliminarAjax()
	{
   		 Ext.Ajax.request(
			{
      			url : '?r=Cargo/Delete',
		 params:{ 
        		id_cargo:Ext.getCmp('grid').getSelectionModel().getSelected().get('id_cargo')          
				},
			callback:function(a,b,c)
				{
				   if(b)
