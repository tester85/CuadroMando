...
{  
	Ext.Ajax.request({
     url : '?r=Riesgo/BuscarRiesgos',	   
     params:{  
			id_indicador: Ext.getCmp('combo_search_indicadores').getValue()			 
			},	
		callback: function(a,b,c)
		{ 			
			if(b)
			{  
				if(c.responseText == "false")
				{
					Ext.Msg.show({
							   title:'Informaci&oacute;n de error',
							   msg: 'No se encontraron riesgos asociados al indicador',
							   buttons: Ext.Msg.OK,
							   //fn: processResult,
							   animEl: 'elId',
							   icon: Ext.MessageBox.ERROR  
						  ...
						  store.load();
						  combo_search_indicadores.reset();
				}
				else
				{
					var dd = c.responseText; 
...
var combo_search_indicadores =  new Ext.form.ComboBox({
	id:'combo_search_indicadores', 
	name:'combo_search_indicadores',
	displayField: 'nombre_indicador',
	valueField:'id_indicador',	
	width: 300,
	minHeight:200, 
...
//************************************* Panel donde se carga la informacion del index 
		var panel = new Ext.FormPanel({
			...
			items: [
			 {   
					tbar:[btnadicionar,btnmodificar,btneliminar,'->','Filtro de indicadores:  ',combo_search_indicadores]//,'->',btnAsociarArea]			
